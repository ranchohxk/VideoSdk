 ...
 * 修改内容: Face功能，需要加密
 */
public class FaceCalManager {
    private static FaceCalManager mFaceCalManager;
    private Context mContext;
    private OnFaceCalListener mFaceCalListener;
    private long mLastDetectTime = 0;


    public FaceCalManager(Context context) {
        mContext = context;
    }

    public static FaceCalManager getInstance(Context context) {
        if (mFaceCalManager == null) {
            mFaceCalManager = new FaceCalManager(context);
        }
        return mFaceCalManager;
    }
    ...
    public void faceCalculate(List<Face> faces, GLSurfaceView glSurfaceView) {
        if (faces.size() == 0) {
